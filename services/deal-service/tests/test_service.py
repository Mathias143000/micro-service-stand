...
import app.server as deal_server


class DealServiceTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        deal_server.publish_event = lambda *_args, **_kwargs: (True, "published")
        deal_server.rabbitmq_ready = lambda: True
        cls.server = deal_server.create_server("127.0.0.1", 0)
    ...
    def test_create_viewing_request(self) -> None:
        status, body = self.post("/viewing-requests", {"listing_id": 1, "visitor_email": "demo@example.com"})
        self.assertEqual(status, 201)
        self.assertTrue(body["event_published"])
        self.assertEqual(body["item"]["listing_id"], 1)
    ...
    def test_metrics_endpoint(self) -> None:
        ...
        self.assertIn("viewing_requests_total", body)
