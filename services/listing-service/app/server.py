...
def metrics_payload() -> str:
    lines = [
        ...
        "# HELP listings_total Current number of listings in memory",
        "# TYPE listings_total gauge",
        f"listings_total {len(LISTINGS)}",
        "# HELP listing_events_published_total RabbitMQ events published by type",
        "# TYPE listing_events_published_total counter",
    ]
    ...
    for event_type, count in sorted(EVENT_COUNTS.items()):
        lines.append(f'listing_events_published_total{{event_type="{event_type}"}} {count}')
...
def ensure_queue() -> bool:
    try:
        queue_name = urllib.parse.quote(EVENT_QUEUE, safe="")
        rabbitmq_request(
            f"queues/%2F/{queue_name}",
            method="PUT",
        ...
        if path == "/":
            self._send_json(
                200,
                {
                    ...
                    "slug": SERVICE_SLUG,
                    "status": "ok",
                    "endpoints": ["/health", "/ready", "/live", "/metrics", "/listings", "/stats"],
                },
            )
        elif path == "/health":
            ...
            self._send_json(200, {"items": LISTINGS, "count": len(LISTINGS)})
        elif path == "/stats":
            self._send_json(200, {"listings_total": len(LISTINGS), "events_published": dict(EVENT_COUNTS)})
        else:
            status_code = 404
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path != "/listings":
            ...
            with DATA_LOCK:
                listing = {
                    "id": NEXT_ID,
                    "title": str(payload.get("title") or f"Demo Listing {NEXT_ID}"),
                    "city": str(payload.get("city") or "Moscow"),
                    "price": int(payload.get("price") or 10000000),
                    "status": "published",
                }
                NEXT_ID += 1
                LISTINGS.append(listing)

            published, detail = publish_event("listing.created", listing)
            self._send_json(
                201,
                {
                    "item": listing,
